...
int visited[max];//dijkstra
int marked[max];//dijkstra and backtracking
...
void read(int n)
{
	...
	while(1)
	{
		printf("Enter the vertices connected by the edge and the weight associated with the respsective edge.\n");
...
void addv(int n)
{
	...
	while(1)
	{
		printf("Enter the number of the vertex connected to vertex number %d, followed by the weight associated with edge.\n", n);
		printf("To end this process, press 0.\n");
		scanf("%d", &j);
		if(j==0)
			break;
...
void change(int n)
{
	...
	switch(k)
	{
		case 1:
		{
			printf("Enter the vertices connected by the new edge.\n");
			scanf("%d%d", &j, &y);
			if(checkvertex(y, n) && checkvertex(j, n))
			{
		...
		case 2:
		{
			printf("Enter the vertices connected by the edge.\n");
			scanf("%d%d", &j, &y);
			if(checkvertex(y, n) && checkvertex(j, n))
			{
