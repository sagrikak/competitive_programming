...
void read(int n, int x)
{
	...
	for(i=x; i<n; i++)
	{
		...
		head=NULL;
		printf("Enter the vertices vertex number %d is connected to.\nEnter 0 when finished.\n", i+1);
		while(1)
		{
			scanf("%d", &j);
			if(checkvertex(j, n))
...
void change(int j, int n)
{
	...
	if(k==1)
	{
		printf("Enter the number of the vertex you want to connect to vertex number %d.\n", j);
		scanf("%d", &x);
		if(checkvertex(x, n))
		{
			if(arr[j-1][k-1]==0)
			...
			if(x!=j)
				list[x-1]=insertnode(j, list[x-1]);
		}
		else return;
	}
	else if(k==0)
	{
		printf("Enter the number of the vertex you want to disconnect to vertex number %d.\n", j);
		scanf("%d", &x);
		if(checkvertex(x, n))
		{
			if(arr[j-1][x-1]==1)
			...
			{
				printf("The connection you are trying to break doesn't exist.\n");
				return;
			}
			temp=list[j-1];
			if(temp->data==x)
